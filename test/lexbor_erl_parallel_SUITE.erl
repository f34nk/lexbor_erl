...
-export([test_parallel_parse_serialize/1, test_parallel_stateful_ops/1,
         test_parallel_mixed_ops/1, test_worker_isolation/1]).
...
test_worker_isolation(_Config) ->
    ...
    receive
        {done, 1} ->
            ok
    after 5000 ->
        ct:fail(timeout_doc1)
    end,
    receive
        {done, 2} ->
            ok
    after 5000 ->
        ct:fail(timeout_doc2)
    end,
    receive
        {done, 3} ->
            ok
    after 5000 ->
        ct:fail(timeout_doc3)
    end,
