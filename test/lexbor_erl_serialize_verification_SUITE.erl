...
-export([
    test_serialize_with_doctype/1,
    test_serialize_without_doctype/1,
    test_serialize_roundtrip/1,
    test_serialize_output_format/1
]).
...
test_serialize_with_doctype(_Config) ->
    ...
    ct:pal("~nCurrent behavior: DOCTYPE is ~s in serialization",
           [case HasDoctype of true -> "INCLUDED"; false -> "NOT INCLUDED" end]),
...
test_serialize_roundtrip(_Config) ->
    ...
    Original = <<"<!DOCTYPE html><html><head><title>Test</title></head>"
...
test_serialize_output_format(_Config) ->
    ...
    TestCases = [
        {<<"<!DOCTYPE html><html><head></head><body>Simple</body></html>">>,
         "Simple with DOCTYPE"},
        {<<"<html><head></head><body>Simple</body></html>">>,
         "Simple without DOCTYPE"},
    ...
    lists:foreach(fun({Html, Description}) ->
        ...
        FirstChars = case byte_size(Serialized) of
            N when N >= 20 -> binary:part(Serialized, 0, 20);
            N -> binary:part(Serialized, 0, N)
        end,
