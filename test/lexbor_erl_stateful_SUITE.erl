...
-export([test_parse_release/1, test_parse_select_basic/1, test_parse_select_multiple/1,
         test_select_with_classes/1, test_select_with_id/1, test_select_descendant/1,
         test_select_child/1, test_outer_html/1, test_invalid_selector/1, test_release_twice/1,
         test_use_after_release/1, test_empty_html/1, test_large_document/1,
         test_nested_elements/1, test_unicode_content/1]).

all() ->
    [{group, stateful}, {group, errors}, {group, edge_cases}].
...
groups() ->
    [{stateful,
      ...
      [test_parse_release,
       ...
       test_select_child,
       test_outer_html]},
     {errors, [sequence], [test_invalid_selector, test_release_twice, test_use_after_release]},
     {edge_cases,
      [sequence],
      [test_empty_html, test_large_document, test_nested_elements, test_unicode_content]}].
...
test_select_with_classes(_Config) ->
    Html =
        <<"<ul><li class='a'>One</li><li class='b'>Two</li><li class='a'>Three</li></ul>">>,
    {ok, Doc} = lexbor_erl:parse(Html),
